# 👋 Hi, I'm Himanshi

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:312e81,50:6d28d9,100:4c1d95&height=220&section=header&text=Himanshi&fontSize=48&fontColor=ffffff&animation=fadeIn&fontAlignY=35&desc=Software%20Engineer%20%7C%20AI%2FML%20Enthusiast&descAlignY=55&descSize=18" width="100%"/>
</p>

<p align="center">
  <a href="https://readme-typing-svg.demolab.com">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=3000&pause=1000&color=A78BFA&center=true&vCenter=true&width=700&lines=BE+CSE+Student+%40+Chitkara+University;Java+%7C+DSA+%7C+DBMS;Full+Stack+Development;AI%2FML+Enthusiast;Building+Projects+%26+Solving+Problems" alt="Typing SVG"/>
  </a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/BE%20CSE-Student-6D28D9?style=for-the-badge&logo=google-scholar&logoColor=white"/>
  <img src="https://img.shields.io/badge/CGPA-8.45%2F10-A78BFA?style=for-the-badge&logo=openjdk&logoColor=white"/>
  <img src="https://img.shields.io/badge/AI%2FML-Explorer-4C1D95?style=for-the-badge&logo=python&logoColor=white"/>
  <img src="https://img.shields.io/badge/Punjab-India-312E81?style=for-the-badge&logo=googlemaps&logoColor=white"/>
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/himanshi-garg-a61305332">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-6D28D9?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://leetcode.com/u/HimanshiGarg12">
    <img src="https://img.shields.io/badge/LeetCode-Arrays%20%26%20DSA-A78BFA?style=for-the-badge&logo=leetcode&logoColor=white"/>
  </a>
  <a href="https://github.com/himanshigarg12">
    <img src="https://img.shields.io/badge/GitHub-himanshigarg12-4C1D95?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=himanshigarg12&label=PROFILE%20VIEWS&color=6d28d9&style=for-the-badge"/>
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=0:4c1d95,100:6d28d9&height=2&width=100%"/>
</p>

## 🚀 About Me

I'm a Computer Science Engineering student at **Chitkara University** passionate about **software engineering, full-stack development, and AI/ML**.

I enjoy building real-world applications with **React, Node.js, and Python**, while continuously strengthening my foundations in **Data Structures & Algorithms, DBMS, SQL, and Object-Oriented Programming**.

<table align="center">
<tr>
<td width="50%" valign="top">

### 🎯 What I Focus On
- 💻 Building practical full-stack projects
- ☕ Strengthening **Java & DSA** (LeetCode — Arrays focus)
- 🗄️ Working with **PostgreSQL & MongoDB**
- 🌐 Developing full-stack applications with React & Node.js
- 🤖 Exploring **AI/ML** — sentiment analysis, prediction models, recommendation systems
- 🧩 Solving programming problems consistently

</td>
<td width="50%" valign="top">

### 🌱 Open To
- 💼 Software Engineering Internships
- 🌐 Full Stack Development Opportunities
- 🤖 AI/ML Projects
- 🔓 Open Source Contributions
- 🏆 Hackathons & Technical Collaborations

</td>
</tr>
</table>

---

## 🛠️ Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=java,python,cpp,js,html,css"/>
</p>

<table align="center">
<tr>
<td align="center" valign="top" width="33%">

**Frontend**
<br/>
<img src="https://skillicons.dev/icons?i=html,css,js,react"/>

</td>
<td align="center" valign="top" width="33%">

**Backend & Databases**
<br/>
<img src="https://skillicons.dev/icons?i=nodejs,express,flask,mongodb,postgres"/>

</td>
<td align="center" valign="top" width="33%">

**Tools & Development**
<br/>
<img src="https://skillicons.dev/icons?i=git,github,vscode,linux,jupyter"/>

</td>
</tr>
</table>

---

## 🧠 AI / ML Expertise

| Domain | Focus | Details |
|---|---|---|
| 🤖 Supervised Learning | 🟣 Applied | Naïve Bayes, Logistic Regression, Random Forest, Decision Trees |
| 🧠 NLP & Text Analytics | 🟣 Applied | Sentiment Analysis, TF-IDF, Cosine Similarity (89% F1-score) |
| 🧩 Unsupervised Learning | 🟣 Applied | K-Means Clustering for mood-based recommendations |
| 🧹 Data Processing | 🟣 Developing | Pandas & NumPy — cleaning and feature engineering |
| 🚀 Model Deployment | 🟣 Developing | Flask ML microservices |
| 👁️ Computer Vision | 🟣 Applied | OpenCV for real-time sign-language decoding |

---

## 🚀 Featured Projects

<table align="center" width="100%">
<tr>
<td width="50%" valign="top">

### 🎬 FilmFusion AI
Intelligent Indian entertainment platform combining Bollywood/Hollywood/OTT discovery with AI-powered sentiment analysis, hit/flop prediction, and mood-based recommendations.

**Stack:** React, Node.js, Express, MongoDB, Python Flask, Scikit-learn, TF-IDF, TMDB API, NewsAPI
**Focus:** Full Stack + AI/ML
**Highlight:** 89% F1-score sentiment analysis model

[🔗 View Repository](https://github.com/himanshigarg12)

</td>
<td width="50%" valign="top">

### 🤟 IoT-Based Text to Speech and Sign Language Decoder
An assistive IoT system converting text into speech and sign language to support inclusive communication.

**Stack:** Raspberry Pi, Python, OpenCV
**Focus:** Embedded Systems + Computer Vision
**Recognition:** 🏆 Secured 4th Position

[🔗 View Repository](https://github.com/himanshigarg12)

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🎓 Quorum — University Social Media Platform
A full-stack social media platform for university students with secure authentication, profile management, and a dynamic content feed.

**Stack:** HTML, CSS, JavaScript, React, Node.js
**Focus:** Full Stack Development
**Highlight:** Session management & API-driven data flow

[🔗 View Repository](https://github.com/himanshigarg12)

</td>
<td width="50%" valign="top">

### 🔧 More Coming Soon
Actively building new full-stack and AI/ML projects — check my GitHub for the latest.

**Focus:** Continuous Learning & Building

[🔗 Explore GitHub](https://github.com/himanshigarg12)

</td>
</tr>
</table>

---

## 💻 Problem Solving

<p align="center">
  <a href="https://leetcode.com/u/HimanshiGarg12">
    <img src="https://img.shields.io/badge/LeetCode-Solving%20Consistently-6D28D9?style=for-the-badge&logo=leetcode&logoColor=white"/>
  </a>
  <img src="https://img.shields.io/badge/Focus-Arrays%20%26%20DSA-A78BFA?style=for-the-badge&logo=openjdk&logoColor=white"/>
</p>

<p align="center">
<code>Arrays</code> <code>Strings</code> <code>Recursion</code> <code>Trees</code> <code>Heaps</code> <code>Generics</code> <code>Collections</code> <code>OOP</code> <code>Complexity Analysis</code>
</p>

---

## 🏆 Achievements & Milestones

<p align="center">

| Recognition | Details |
|---|---|
| 🏆 Project Recognition | 4th position — IoT-Based Text to Speech and Sign Language Decoder |
| 📜 Certifications | Certified courses completed on Coursera |
| 🧩 Hackathons | Advanced to final rounds in multiple hackathons |
| 🤝 Community | Active member — NSS, IEEE, IEEE CIET Coding Club |

</p>

---

## 📊 GitHub Analytics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=himanshigarg12&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=A78BFA&icon_color=8B5CF6" height="165"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=himanshigarg12&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=A78BFA" height="165"/>
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=himanshigarg12&theme=tokyonight&hide_border=true&background=0D1117&ring=A78BFA&fire=8B5CF6&currStreakLabel=A78BFA" width="65%"/>
</p>

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=himanshigarg12&theme=discord&no-frame=true&no-bg=true&margin-w=6&row=1&column=6"/>
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=himanshigarg12&bg_color=0D1117&color=A78BFA&line=8B5CF6&point=C4B5FD&area=true&hide_border=true" width="95%"/>
</p>

---

## 🎯 Current Focus

```yaml
Learning:   Advanced DSA · Backend Development · Machine Learning
Building:   AI-powered Recommendation Systems · Full Stack Apps
Exploring:  XGBoost · Streamlit Dashboards · PostgreSQL-backed Applications
Open To:    Internships · AI/ML Collaborations · Hackathons
```

---

## 🤝 Connect With Me

<p align="center">
  <a href="mailto:himanshi0409.becse24@chitkara.edu.in">
    <img src="https://img.shields.io/badge/Gmail-6D28D9?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://www.linkedin.com/in/himanshi-garg-a61305332">
    <img src="https://img.shields.io/badge/LinkedIn-6D28D9?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://leetcode.com/u/HimanshiGarg12">
    <img src="https://img.shields.io/badge/LeetCode-4C1D95?style=for-the-badge&logo=leetcode&logoColor=white"/>
  </a>
  <a href="https://github.com/himanshigarg12">
    <img src="https://img.shields.io/badge/GitHub-312E81?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

---

<p align="center">
  <b>✨ "Build consistently. Solve problems. Keep learning." ✨</b>
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:4c1d95,50:6d28d9,100:312e81&height=120&section=footer"/>
</p>
